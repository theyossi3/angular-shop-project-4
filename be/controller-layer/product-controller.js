...
const Verify = require('../middleware/verify-logged-in');
...
router.patch("/:id", async (request, response) => {
    ...
    try {
        ...
        const updatedProduct = await product_dal.patchProduct(productId, request.body, image);
...
router.get("/getbycategories/:id", async (request, response) => {
    try {

        const id = request.params.id;
        const products = await product_dal.getbycategories(id);
